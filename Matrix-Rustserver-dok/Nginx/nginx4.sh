...
# 定义 Nginx 配置相关变量
CONFIG_FILE="/root/Proxy/Nginx/other/other.conf"
...
function tail_nginx_logs() {
  if docker ps -q -f name="$CONTAINER_NAME" > /dev/null; then
    echo "📜 正在实时显示 Nginx 容器 $CONTAINER_NAME 的日志..."
    echo "按 Ctrl+C 返回主菜单"
    log "INFO" "开始实时查看 Nginx 容器 $CONTAINER_NAME 的日志"
    ...
    clear
    tput cup 0 0
    echo "📜 Nginx 容器 $CONTAINER_NAME 日志（按 Ctrl+C 返回主菜单）："

    # 捕获 Ctrl+C 以返回主菜单
    trap 'clear; echo -e "\nℹ️ 返回主菜单"; log "INFO" "用户中断查看 Nginx 日志"; return' SIGINT

    # 初始化一个数组来存储日志行
    declare -a log_buffer
    local buffer_size=$log_lines

    # 使用 docker logs -f 实时读取日志
    docker logs -f --tail "$log_lines" "$CONTAINER_NAME" 2>&1 | while IFS= read -r line; do
      # 将新日志添加到缓冲区
      log_buffer+=("$line")
      
      # 如果缓冲区超过指定行数，移除最早的行
      if [ ${#log_buffer[@]} -gt "$buffer_size" ]; then
      ...
      # 显示当前缓冲区的所有日志
      for ((i=0; i<${#log_buffer[@]}; i++)); do
        echo "${log_buffer[$i]}"
      done
    done
  else
    echo "⚠️ Nginx 容器 $CONTAINER_NAME 未运行，无法查看日志"
    log "WARN" "Nginx 容器 $CONTAINER_NAME 未运行，无法查看日志"
  fi
}
...
while true; do
  ...
  case $CHOICE in
    ...
    9) reload_nginx ;;
    10) tail_nginx_logs ;;
    11) echo "🚪 退出脚本"; log "INFO" "用户退出脚本"; exit 0 ;;
    *) echo "❌ 请输入有效选项 (1-11)！"; log "ERROR" "无效选项: $CHOICE" ;;
  esac
  echo "" # 添加空行提高可读性
done